...
# Assemble driver assembly files (if any)
PWR_ASM="src/stm32/drivers/pwr/pwr_asm.s"
if [ -f "$PWR_ASM" ]; then
    echo "Assembling pwr_asm.s..."
    arm-none-eabi-as -mcpu=cortex-m4 -mthumb -mfloat-abi=hard -mfpu=fpv4-sp-d16 "$PWR_ASM" -o build/pwr_asm.o
fi
...
EXTRA_OBJS=""
if [ -f "build/pwr_asm.o" ]; then
    EXTRA_OBJS="$EXTRA_OBJS build/pwr_asm.o"
fi

# Link all object files (using auto-generated linker script)
# main.o comes first so its strong symbols override startup.o weak symbols
echo "Linking..."
